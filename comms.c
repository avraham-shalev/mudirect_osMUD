...
size_t write_data(void *buffer, size_t size, size_t nmemb, void *userp)
{
	...
	logMsg(OMS_DEBUG, "getOpenMudFile::: curl: in write data");
...
int getOpenMudFile(char *mudFile, char *outputFile)
{
	...
	sprintf(msgBuf, "getOpenMudFile:::START. Trying to download URL[%s] to local file[%s])",mudFile,outputFile); 
	...
	if(curl)
	{
		...
		// DOTO: If this takes a long time, it can't block the rest of the app
		//       may need this to be a new thread or something of the sort
		retval = curl_easy_perform(curl);

		if(retval != CURLE_OK)
		{
//			sprintf(msgBuf, "getOpenMudFile:curl_easy_perform() failed: %s", curl_easy_strerror(retval));
			logMsg(OMS_ERROR, "getOpenMudFile:::ERROR: curl_easy_perform() failed");
		}
		else
		{
			logMsg(OMS_DEBUG, "getOpenMudFile::: curl_easy_perform() success. File Was Downloaded Successfully");
		}
	...
	logMsg(OMS_DEBUG, "getOpenMudFile::: END");
	return retval;
}
