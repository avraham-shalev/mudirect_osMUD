...
MudFileInfo* parseMudFile(char *mudFileWithPath) {
	logMsg(OMS_DEBUG, "parseMudFile:::START");
	MudFileInfo *mfi = createMfi();
	char * string = readFileToString(mudFileWithPath);
	logMsg(OMS_DEBUG, "parseMudFile:::Finished with reading MUD File. Creating json object on file content.");
	json_object * jobj = json_tokener_parse(string);
	safe_free(string);
	logMsg(OMS_DEBUG, "parseMudFile:::Finished making json object. Proccessing it..");
	processJson(jobj, "<root>", 0, 0, mfi);
	logMsg(OMS_DEBUG, "parseMudFile::: END. Finished processing json object.");

	return mfi; //TODO: free this object? what is it? need to read about it!
}
