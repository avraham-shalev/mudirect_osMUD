...
const char* getSeverityText(enum OMS_SEVERITY_CLASSES severity)
{
	switch (severity)
	{
		...
		case OMS_VERBOSE: return "VERBOSE";
...
int getLogLevelFromArg(char *logLevel)
{
	...
	if (!strcmpi(logLevel, "INFO"))
		...
		return OMS_DEBUG;
	else if (!strcmpi(logLevel, "VERBOSE"))
		return OMS_VERBOSE;
	else
		return OMS_INFO;
}
...
void logMsg(int severity, char * messageText)
{
	if (severity <= omsLogLevel) {
		time_t ltime;
		ltime=time(NULL);
		...
		fprintf(logFile, "%04d-%02d-%02d %02d:%02d:%02d %s::%s\n", tm->tm_year+1900, tm->tm_mon,
				tm->tm_mday, tm->tm_hour, tm->tm_min, tm->tm_sec, getSeverityText(severity), 					messageText);
		fflush(logFile);
	}
}
