...
DomainResolutions *_domainsArr[MAX_UDOMAINS];
int _idx = 0;
...
void unlinkDeviceFromItsCompany(DhcpEvent *dhcpEvent)
{
	...
	if(dnsRes->deviceCount > 1) /* if there are more devices in the list, copy the last instead of the deleted */
	{
		dnsRes->deviceList[i] = dnsRes->deviceList[dnsRes->deviceCount-1];
	}
	dnsRes->deviceCount--;
...
void *startResolveUniqueDomains(void *param)
{
	...
	{
		acquireLock();
		logMsg(OMS_VERBOSE, "startResolveUniqueDomains::: waken-up, not locked!");
		for (i = 0; i < _idx; i++)
		{
...
void handleDomain(DomainResolutions *dnsRes)
{
	...
	format = "handleDomain::: Unlocked UDomain::Company[%s],domainName[%s],dnsServer[%s]";
	sprintf(msgBuf, format, dnsRes->company, dnsRes->domainName, dnsRes->dnsServer);
	logMsg(OMS_VERBOSE, msgBuf);

	if(resChanged)
	{ /* Remove all relevant devices rules and add them again with all new IP Addresses of this unique domain */
		format = "handleDomain::: Updating [%d] relevant devices with new IP rules!";
		sprintf(msgBuf, format, dnsRes->deviceCount);
		logMsg(OMS_INFO, msgBuf);

		for(int i=0;i < dnsRes->deviceCount;i++)
		{
			format = "handleDomain::: Updating device #[%d] from total #[%d] relevant devices";
...
int isDomainResolutionChanged(char *ipList[15], int ipCount, char *newIpList[15], int ncount)
{
	...
	if(ncount == 0)
	{
		logMsg(OMS_VERBOSE, "isDomainResolutionChanged::: Retreived 0 new IP Addresses, consider it as no-change!!");
		goto END;
	}
	...
	for (int j = 0; j < ncount; j++)
	{
		...
		if (!ipFound)
		{
			format = "isDomainResolutionChanged::: TRUE!!::newIp[%s]";
			sprintf(msgBuf, format, newIpList[j]);
			logMsg(OMS_INFO, msgBuf);
...
void updateDomainWithNewAddresses(DomainResolutions *dnsRes, char *newIpList[], int ncount)
{
	...
	logMsg(OMS_INFO, "updateDomainWithNewAddresses::: START");
	//logMsg(OMS_VERBOSE, "updateDomainWithNewAddresses:::Calling safe_free()");
/*	free all existing IPs strings from memory */
	/*for (int i = 0; i < dnsRes->ipCount; i++)
	{
		safe_free(dnsRes->ipList[i]);
	}
	logMsg(OMS_VERBOSE, "updateDomainWithNewAddresses:::After calling safe_free()");
*/
	/* update */
	...
	for (int j = 0; j < ncount; j++)
	{
		...
		for (int i = 0; i < dnsRes->ipCount; i++)
		{
			if (strcmp(dnsRes->ipList[i], newIpList[j]) == 0)
			{
				ipFound = 1;
				break;
			}
		...
		if (!ipFound)
		{
			dnsRes->ipList[dnsRes->ipCount++] = newIpList[j];
		}
	}
